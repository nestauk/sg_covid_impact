# %%
"""Utility functions for GoogleTrends flow.

Contains the core class of the GTAB package (until we have a proper fork).
This stops some of the annoying behaviour that gets in the way of automation 
such as asking for user input when an exception should be raised, e.g. if
a request to the API returns 429.
...
def wait_longer_after_one_failure(retry_state):
    """Quickly retry first time, then wait a minute."""
...
@retry(
    wait=wait_longer_after_one_failure,
    retry=(
...
def gtab_query(word: str, gtab: GTAB_) -> pd.Series:
    """Query gtab with `word`

    Retries up to 3 times on ConnectionError or Response Error.
    The first retry is fast, but the subsequent retries happen after a minute.
    """
    word_clean = re.sub("_", " ", word)  # Remove underscores
    try:
        result = gtab.new_query(word_clean)
        return result["max_ratio"].rename(word_clean)
    except ConnectionError as e:
        if "429" not in e.args[0]:
            raise Exception(f"Unhandled ResponseError: {e.args[0]}")
        ...
        # Deal with exit codes by just passing through
        # TODO: be more explicit about this. how?
        return pd.Series([result], name=word_clean)


def set_gtab_timeframe(gtab: GTAB_, anchor_period: str) -> str:
...
def anchor_period_to_anchor_filename(anchor_period: str) -> str:
    """Filename corresponse to `anchor_period` in Great Britain."""
    return f"google_anchorbank_geo=GB_timeframe={anchor_period}.tsv"


class GTAB:
    """GTAB core class - here be dragons!"""
    ...
    def __init__(self, dir_path=None, from_cli=False):
        ...
        if dir_path is None:
            self.dir_path = GTAB_().dir_path  # XXX: HACK
        else:
...
if __name__ == "__main__":
    gtab = GTAB()
    gtab.set_options(conn_config={"retries": 0})

    gtab_query("react", gtab)
